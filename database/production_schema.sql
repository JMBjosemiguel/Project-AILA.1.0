...
-- migration 006: gamification. `achievements` = catalog (badge = an achievement's
-- icon); `user_achievements` = unlocks, UNIQUE per (user, achievement).
...
-- Attempts carry a lifecycle (migration 002): 'in_progress' while being taken,
-- 'submitted' once graded. active_slot = 1 for an in-progress attempt, NULL
-- otherwise, so UNIQUE(user_id, quiz_id, active_slot) allows one active attempt
-- per quiz plus unlimited historical submitted attempts.
...
-- is_correct is NULL until the attempt is submitted and graded server-side.
-- UNIQUE(attempt_id, question_id) makes answer autosave an idempotent UPSERT.
